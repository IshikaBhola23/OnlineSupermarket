...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			int addedwalletprice=Integer.parseInt(request.getParameter("addedmoney"));
			User user=(User)session.getAttribute("active-user");
			int userId=user.getUserId();
			int addMon=Integer.parseInt(user.getUserPhone())+ addedwalletprice;
			boolean flag=new UserDaoImp().addmoneytowallet(userId, addMon);
			if(flag==true)
			{
				session.setAttribute("message","Please Verify your account to add money!");
		        response.sendRedirect("login.jsp");
		        session.removeAttribute("active-user");
			}
			...
			{
				session.setAttribute("message","Failed to add money");
		        response.sendRedirect("index.jsp");
			}
		}
